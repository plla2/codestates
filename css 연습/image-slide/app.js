...
const elementsToGlich = [
  ...document.querySelectorAll("main > span"),
  ...document.querySelectorAll(".footer > div > span:nth-of-type(2)"),
];
...
function glich() {
  ...
  const interval = setInterval(() => {
    elementsToGlich.forEach((el) => {
      el.children[el.children.length - 1].innerText = randomString(
        el.children[0].innerText.length
      );
    });
  }, 50);
